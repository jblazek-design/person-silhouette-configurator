...
// ---- story headline artwork (optional): socialmind/assets/story-title.svg ----
// Drawn at its natural size, centred, at the top of the Instagram story export.
let storyTitle = null;
const titleFile = path.join(ROOT, 'assets', 'story-title.svg');
if (fs.existsSync(titleFile)) {
  const svg = fs.readFileSync(titleFile, 'utf8');
  const vb = svg.match(/viewBox="0 0 ([\d.]+) ([\d.]+)"/);
  if (!vb) throw new Error('story-title.svg needs a viewBox starting at 0 0');
  let s = svg.match(/<svg[^>]*>([\s\S]*)<\/svg>/)[1];
  s = s.replace(/stroke="black"/g, `stroke="${INK}"`).replace(/fill="black"/g, `fill="${INK}"`);
  s = s.replace(/\sid="([^"]+)"/g, ' id="__ID__$1"').replace(/url\(#([^)]+)\)/g, 'url(#__ID__$1)');
  s = s.replace(/\s+/g, ' ').replace(/>\s+</g, '><').trim();
  storyTitle = { w: parseFloat(vb[1]), h: parseFloat(vb[2]), svg: `<g fill="none">${s}</g>` };
  total += 1;
}
...
window.SM_PARTS = ${JSON.stringify({ frame: FRAME, ink: INK, body: BODY, tones, shapes, parts, storyTitle })};
`;
